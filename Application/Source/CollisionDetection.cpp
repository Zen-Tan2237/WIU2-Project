...
glm::vec3 SafeNormalize(const glm::vec3& vec, const glm::vec3& fallback)
{
...
void ResolveCollision(CollisionData& cd)
{
	...
	// Calculate friction coefficient (combine both objects)
	float mu = (A->friction + B->friction) * 0.5f;

	// Calculate tangent directions for friction (perpendicular to collision normal)
	glm::vec3 tangent = cd.tangent;   // Already generated by collision detection
	glm::vec3 bitangent = cd.bitangent; // Already generated by collision detection

	// Project relative velocity onto tangent and bitangent
	...
	// Friction impulse along bitangent
	...
	float jb = -velAlongBitangent;
	if (denomBitangent > 1e-6f)
		jb /= denomBitangent;
	else
		jb = 0.0f;

	// Apply Coulomb friction model: |friction impulse| <= mu * |normal impulse|
	// This limits friction based on the normal force
	float maxFrictionMagnitude = mu * jn;
	float frictionMagnitude = std::sqrt(jt * jt + jb * jb);

	if (frictionMagnitude > maxFrictionMagnitude && frictionMagnitude > 1e-6f) {
		// Clamp friction to maximum allowed value
	...
	// Total impulse: normal + friction components
	...
	// Calculate and apply angular impulses
	if (invMassA > 0.0f) {
...
void ResolveCollision(CollisionData& cd, float dt)
{
	// Get the two physics objects
	PhysicsObject* A = cd.pObjA;
	PhysicsObject* B = cd.pObjB;
	...
	float bias = 0.0f;
	if (dt > 1e-6f) {
		bias = (BETA / dt) * std::max(cd.penetration - SLOP, 0.0f);
	}
...
void ApplyPositionCorrection(CollisionData& cd)
{
	...
	const float MAX_CORR = 0.20f; // cap per frame

	float depth = std::max(penetration - SLOP, 0.0f);
	depth = ClampFloat(depth * PERCENT, 0.0f, MAX_CORR);
	if (depth <= 0.f) return;

	glm::vec3 correction = normal * (depth / invMassSum);
	A->position -= correction * invMassA;
	B->position += correction * invMassB;
}
