...
void ResolveCollision(CollisionData& cd)
{
	...
	// Only apply once with conservative settings
	const float SLOP = 0.01f;
	const float PERCENT = 0.15f; // REDUCED from 0.2f - less aggressive
	const float MAX_CORRECTION = 0.3f; // REDUCED from 0.5f
	...
	// ==================== VELOCITY THRESHOLD ====================
	// If objects are barely moving into each other, skip impulse (prevents micro-vibrations)
	const float VELOCITY_THRESHOLD = 0.001f;
	if (velAlongNormal > -VELOCITY_THRESHOLD) return;
	...
	// Calculate bounciness coefficient - REDUCED for stability
	float e = (A->bounciness + B->bounciness) * 0.5f;
	e = std::max(e * 0.6f, 0.0f); // REDUCED from 0.8f to 0.6f
