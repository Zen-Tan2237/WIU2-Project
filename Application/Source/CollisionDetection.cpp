...
void ResolveCollision(CollisionData& cd)
{
	...
	// If both are static, no collision response needed
	if (invMassSum < 1e-6f) return;

	// Calculate relative velocity at contact point FIRST to check if already resting
	...
	float relVelMagnitude = glm::length(relVel);

	// ==================== RESTING CONTACT CHECK ====================
	// If objects are barely moving AND barely penetrating, just do positional correction
	const float REST_VELOCITY_THRESHOLD = 0.05f;  // Objects moving < 0.05 units/sec
	const float REST_PENETRATION_THRESHOLD = 0.005f; // Penetration < 0.5 cm

	bool isRestingContact = (relVelMagnitude < REST_VELOCITY_THRESHOLD) &&
		(penetration < REST_PENETRATION_THRESHOLD);

	if (isRestingContact) {
		// Only apply positional correction for resting contacts
		const float SLOP = 0.001f;
		const float PERCENT = 0.3f;
		const float MAX_CORRECTION = 0.05f;

		if (penetration > SLOP) {
			float correction = (penetration - SLOP) * PERCENT / invMassSum;
		...
		return; // ✅ Skip impulse resolution for resting objects
	}

	// ==================== ACTIVE COLLISION (moving objects) ====================

	// For moving objects, apply full collision resolution
	const float SLOP = 0.001f;
	const float PERCENT = 0.2f;
	const float MAX_CORRECTION = 0.1f;
	...
	e = std::max(e * 0.4f, 0.0f); // ✅ REDUCED for more damping
	...
	glm::vec3 tangent = cd.tangent;
	glm::vec3 bitangent = cd.bitangent;

	float velAlongTangent = glm::dot(relVel, tangent);
	float velAlongBitangent = glm::dot(relVel, bitangent);
	...
	float denomNormal = invMassSum + rotationalA + rotationalB;

	if (denomNormal < 1e-6f) return;
	...
	float rotationalAT = glm::dot(raCrossT, invInertiaA * raCrossT);
	float rotationalBT = glm::dot(rbCrossT, invInertiaB * rbCrossT);
	float denomTangent = invMassSum + rotationalAT + rotationalBT;

	float jt = 0.0f;
	if (denomTangent > 1e-6f) {
		jt = -velAlongTangent / denomTangent;
	}

	glm::vec3 raCrossB = glm::cross(ra, bitangent);
	glm::vec3 rbCrossB = glm::cross(rb, bitangent);
	float rotationalAB = glm::dot(raCrossB, invInertiaA * raCrossB);
	float rotationalBB = glm::dot(rbCrossB, invInertiaB * rbCrossB);
	float denomBitangent = invMassSum + rotationalAB + rotationalBB;

	float jb = 0.0f;
	if (denomBitangent > 1e-6f) {
		jb = -velAlongBitangent / denomBitangent;
	}

	// Apply Coulomb friction model
	float maxFrictionMagnitude = mu * std::max(jn, 0.0f);
	float frictionMagnitude = std::sqrt(jt * jt + jb * jb);

	if (frictionMagnitude > maxFrictionMagnitude && frictionMagnitude > 1e-6f) {
		float scale = maxFrictionMagnitude / frictionMagnitude;
		jt *= scale;
		jb *= scale;
	}
	...
	glm::vec3 impulse = (normal * jn) + (tangent * jt) + (bitangent * jb);
