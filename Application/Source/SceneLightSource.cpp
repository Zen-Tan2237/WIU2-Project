...
void SceneLightSource::Init()
{
	...
	meshList[GEO_PYRAMID] = MeshBuilder::GenerateHemisphere("Pyramid", glm::vec3(1.f, 1.f, 1.f), 1.f, 4, 0.5f);
...
void SceneLightSource::Update(double dt)
{
...
void SceneLightSource::Render()
{
	...
	if (light[0].type == Light::DIRECTIONAL) {
		// For directional light, we need to pass in the direction vector
	...
	{
		PushPop randomahhflatplaneGuard(modelStack);
		modelStack.Translate(0.f, -1.f, 0.f);
		modelStack.Rotate(-90.f, 1.f, 0.f, 0.f);
		modelStack.Scale(50.f, 50.f, 1.f);
		meshList[GEO_QUAD]->material.kAmbient = glm::vec3(0.5f, 0.5f, 0.5f);
		meshList[GEO_QUAD]->material.kDiffuse = glm::vec3(0.85f, 0.85f, 0.85f);
		meshList[GEO_QUAD]->material.kSpecular = glm::vec3(0.1f, 0.1f, 0.1f);
		meshList[GEO_QUAD]->material.kShininess = 5.0f;
		RenderMesh(meshList[GEO_QUAD], enableLight);
	}
}

void SceneLightSource::Exit()
{
	// Cleanup VBO here
	for (int i = 0; i < NUM_GEOMETRY; ++i)
	{
...
void SceneLightSource::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void SceneLightSource::HandleKeyPress() 
{
	...
	{
		int type = light[0].type;
		type = (type + 1) % 3;
		light[0].type = static_cast<Light::LIGHT_TYPE>(type);
		glUniform1i(m_parameters[U_LIGHT0_TYPE],
			light[0].type);
	}
