...
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, glm::vec3 color, float length)
{
	...
	v.pos = glm::vec3(0.5f * length, -0.5f * length, 0.f); v.color = glm::vec3(color.x / 255.f, color.y / 255.f, color.z / 255.f);
...
{
	// Pseudocode:
	// - Initialize vertex/index buffers and a reusable Vertex v
	// - For each slice from 0..numSlice:
	//   - Compute theta and u = slice / numSlice for wrapping texture coordinates
	//   - Create outer vertex:
	//     - pos = (outerR * cos(theta), 0, outerR * sin(theta))
	//     - normal = (0, 1, 0)
	//     - color normalized to [0,1]
	//     - texCoord = (u, 1)
	//     - push to vertex buffer
	//   - Create inner vertex:
	//     - pos = (innerR * cos(theta), 0, innerR * sin(theta))
	//     - normal = (0, 1, 0)
	//     - color normalized to [0,1]
	//     - texCoord = (u, 0)
	//     - push to vertex buffer
	// - Build triangle strip indices in order: 0,1,2,3,... for all slices
	// - Upload buffers to GPU, set indexSize and mode, return mesh
	...
	for (unsigned slice = 0; slice < numSlice + 1; ++slice)
	{
		...
		float u = static_cast<float>(slice) / static_cast<float>(numSlice);

		// Outer vertex
		v.pos = glm::vec3(outerR * cos(theta), 0, outerR * sin(theta));
		v.color = glm::vec3(color.x / 255.f, color.y / 255.f, color.z / 255.f);
	...
	for (unsigned stack = 0; stack < numStack + 1; ++stack) //stack
	{
		...
		for (unsigned slice = 0; slice < numSlice + 1; ++slice) //slice
		{
			...
			v.pos = glm::vec3((outerR + innerR * glm::cos(theta)) * glm::
				...
				glm::cos(phi));
			v.normal = glm::vec3(glm::cos(theta) * glm::sin(phi),
				glm::sin(theta),
...
Mesh* MeshBuilder::GenerateOBJMTL(const std::string& meshName, const std::string& file_path, const std::string& mtl_path) {
	...
	// Index the verts, textcoords & normals properly
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
